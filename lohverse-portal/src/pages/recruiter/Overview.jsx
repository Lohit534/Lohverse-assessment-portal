...
export default function Overview() {
  ...
  // Dynamic AI proctor settings
  const [proctorCamera, setProctorCamera] = useState(() => {
    return JSON.parse(localStorage.getItem('ai_camera_proctor') ?? 'true');
  });
  const [proctorAudio, setProctorAudio] = useState(() => {
    return JSON.parse(localStorage.getItem('ai_audio_proctor') ?? 'true');
  });

  const toggleProctor = (type) => {
    if (type === 'camera') {
      const next = !proctorCamera;
      setProctorCamera(next);
      localStorage.setItem('ai_camera_proctor', JSON.stringify(next));
    } else {
      const next = !proctorAudio;
      setProctorAudio(next);
      localStorage.setItem('ai_audio_proctor', JSON.stringify(next));
    }
  };
  ...
  return (
    <div className="rp-page">
      ...
      {/* AI Proctor Configuration Toggles */}
      <div className="rp-card" style={{ marginBottom: '1.5rem', background: 'linear-gradient(135deg, rgba(124,58,237,0.06), rgba(59,130,246,0.06))', border: '1px solid rgba(167,139,250,0.2)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 800, color: 'var(--r-text)' }}>🤖 Global AI Interview Proctoring Configuration</h3>
            <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.78rem', color: 'var(--r-muted)' }}>Configure whether student camera and microphone are required for Gemini AI interviews.</p>
          </div>
          <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.825rem', fontWeight: 'bold', cursor: 'pointer', color: 'white' }}>
              <input 
                type="checkbox" 
                checked={proctorCamera} 
                onChange={() => toggleProctor('camera')}
