...
export default function Candidates() {
  ...
  useEffect(() => {
    ...
    API.get(`/recruiter/jobs/${selectedJob}/rankings`)
      ...
      .catch(() => toast.error('Failed to load job-specific rankings'))
    ...
    if (selectedJob === 'All') {
      ...
      rows = [
        ...
        ...filteredRankings.map(r => [
          ...
          r.student?.college   || '',
          r.student?.branch    || '',
          r.applicationStatus === 'shortlisted' ? 'Selected' : r.applicationStatus === 'rejected' ? 'Not Selected' : 'Under Review',
        ]),
      ];
    }
  ...
  const handleUpdateStatus = async (studentId, status) => {
    ...
    try {
      ...
      setRankings(prev => prev.map(r => {
        if (r.studentId === studentId) return { ...r, applicationStatus: status };
        return r;
      }));
  ...
  if (loading) return <PageLoader label="Loading candidates database…" />;
    ...
    <div className="rp-page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div className="rp-field" style={{ margin: 0 }}>
          ...
          >
            <option value="All" style={{ background: '#12112a', color: 'var(--r-text)' }}>🔍 Show All Registered Candidates</option>
            {jobs.map(j => (
              <option key={j.id} value={j.id} style={{ background: '#12112a', color: 'var(--r-text)' }}>
                🤖 Rank applicants: {j.title} ({j.companyName})
              </option>
            ))}
          </select>
        </div>
        ...
        {selectedJob === 'All' ? (
          <div className="rp-field" style={{ margin: 0 }}>
            ...
            >
              {branches.map(b => (
                <option key={b} value={b} style={{ background: '#12112a', color: 'var(--r-text)' }}>
